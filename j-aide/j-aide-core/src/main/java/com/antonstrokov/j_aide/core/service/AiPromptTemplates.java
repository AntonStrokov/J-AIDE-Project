...
public final class AiPromptTemplates {
	...
	public static final PromptTemplate TEST_GENERATION_TEMPLATE = PromptTemplate.from(
			"Ты опытный {{language}}-разработчик и Java test engineer.\n" +
					...
					"- Предпочитай JUnit 5.\n" +
					"- Тестируй только публичный интерфейс (public API). Никогда не вызывай приватные (private) методы напрямую и не используй Reflection (рефлексию) для доступа к ним.\n" +
					"- Используй Mockito только если для тестируемого кода явно нужны зависимости или mock-объекты.\n" +
					"- Аннотации тестов (например, @Test) ОБЯЗАТЕЛЬНО пиши на отдельной строке. Запрещено писать @Test и сигнатуру метода на одной строке.\n" +
					"- Если выделенный код состоит только из приватных методов (private), не вызывай их. Вместо этого сгенерируй тестовый класс-каркас и добавь в нём Java-комментарий о том, что приватные методы следует тестировать косвенно через публичный API (public API).\n" +
