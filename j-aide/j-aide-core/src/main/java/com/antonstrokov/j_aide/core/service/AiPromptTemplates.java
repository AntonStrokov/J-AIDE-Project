...
public final class AiPromptTemplates {
	...
	public static final PromptTemplate IMPROVE_TEMPLATE = PromptTemplate.from(
			"Ты опытный {{language}}-разработчик.\n" +
					...
					"- Поле changeFacts должно быть массивом машиночитаемых фактов об изменениях.\n" +
					"- Сейчас в changeFacts описывай только переименование методов.\n" +
					"- Для переименования метода добавляй объект с operation=\"rename\", symbolKind=\"method\", before и after.\n" +
					"- before и after должны содержать точные идентификаторы методов из исходного и улучшенного кода без перевода и перефразирования.\n" +
					"- Если переименований методов нет, changeFacts должен быть пустым массивом [].\n" +
					"- Не добавляй в changeFacts факт переименования, если такого изменения реально нет между исходным кодом и improvedCode.\n" +
					...
					"  \"changeFacts\": [{\"operation\": \"rename\", \"symbolKind\": \"method\", \"before\": \"oldMethodName\", \"after\": \"newMethodName\"}],\n" +
	...
	public static final PromptTemplate TEST_GENERATION_TEMPLATE = PromptTemplate.from(
			"Ты опытный {{language}}-разработчик и Java test engineer.\n" +
					...
					"- Если structuralContext содержит package production-класса, testCode должен корректно учитывать этот package.\n" +
					"- Предпочтительно помещай тест в тот же package, что и production-класс.\n" +
					"- Если тест находится в другом package, добавь корректный import production-класса; не оставляй ссылку на production-класс по простому имени без корректной package/import связи.\n" +
